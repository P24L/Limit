...
struct UserProfileView: View {
    ...
    var body: some View {
        ...
        .onAppear {
            followingURI = actorWrapped.profile?.viewer?.followingURI
        }
    }
                    ...
                    Button {
                        Task {
                            if let uri = followingURI {
                                ...
                                let ref = await client.followActor(actor: did)
                                if let ref {
                    ...
                    Button {
                        Task {
                            if let uri = followingURI {
                                // UNFOLLOW
                                await client.deleteFollowRecord(recordID: uri)
                                onFollowToggle(profile.actorDID, nil)
                                followingURI = nil
                            } else {
                                // FOLLOW
                                let ref = await client.followActor(actor: profile.actorDID)
                                onFollowToggle(profile.actorDID, ref?.recordURI)
                                followingURI = ref?.recordURI
                            }
                        }
                    } label: {
